...
async def create_task(
    db: AsyncSession, project_id: str, data: TaskCreate, current_user: User
) -> Task:
    ...
    if dup:
        raise HTTPException(
            status_code=409, detail="This task already exists in the project."
        )
...
async def update_task(
    db: AsyncSession, task_id: str, data: TaskUpdate, current_user: User
) -> Task:
    task = (
    ...
    if data.assigned_to is not None and task.status == TaskStatus.completed:
        raise HTTPException(
            status_code=422, detail="Completed tasks cannot be reassigned."
        )
...
async def update_task_status(
    db: AsyncSession, task_id: str, data: TaskStatusUpdate, current_user: User
) -> Task:
    task = (
        await db.execute(select(Task).where(Task.id == task_id))
    ).scalar_one_or_none()
...
async def delete_task(db: AsyncSession, task_id: str, current_user: User) -> None:
    ...
    public_ids = (
        (
            await db.execute(
                select(Attachment.cloudinary_public_id).where(
